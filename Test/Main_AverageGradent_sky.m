...
abs_DATA = abs(data-1);
figure;imshow((abs_DATA));title('去噪梯度');
...
Sky_N_ind = find((Sky==0) | (Sky==1));
Sky_Y_ind = find((Sky==2));

Sky_Y = ImgsegProcess(img,Sky_N_ind,m,n); %非天空区域索引赋值为0，输出天空区域
Sky_N = ImgsegProcess(img,Sky_Y_ind,m,n);

alpha=0;
[Out_Y,beta] = DefogProcess(Sky_Y,alpha);
[Out_N,alpha] = DefogProcess(Sky_N,beta);

out = Out_N + Out_Y;
figure;imshow(uint8(out));title('去雾图像');
% %% 融合
